...
<body class="w-full bg-gray-600">
	<form
		class="my-5 w-full lg:w-1/2 text-xl mx-auto text-center bg-gray-300 rounded-lg py-4"
		id="myForm"
		method="POST"
		action="?/add"
	>
		...
		<p class="inline">
			รับมา(เฉพาะยอดใหม่(ทศนิยม2ตำแหน่ง)): <input
				class="w-auto inline p-1 text-sm text-gray-900 bg-gray-400 rounded-lg border border-gray-300"
				type="number"
				step="0.01"
				name="Recived"
				value=""
		...
		<br />
		<p class="inline">
			Goal(60000)(ไม่ต้องใส่ถ้าไม่แก้): <input
				class="w-auto w-fit p-1 text-sm text-gray-900 bg-gray-400 rounded-lg border border-gray-300"
				type="number"
				step="0.01"
				name="Needed"
			/>บาท
